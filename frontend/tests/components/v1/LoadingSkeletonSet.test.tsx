...
} from "../../../src/utils/v1/skeletonUtils";
import {
  SKELETON_PRESETS,
  skHeightThumbnail,
  skHeightTextLg,
...
describe("LoadingState Conditional Rendering", () => {
  it("renders error state when error is provided", () => {
    ...
    expect(screen.getByTestId("skeleton-error").textContent).toContain(
  ...
  it("renders empty state when empty is true and no loading/error", () => {
    ...
    expect(screen.getByTestId("skeleton-empty").textContent).toContain(
      "No data available",
    );
    expect(screen.queryByTestId("content")).toBeNull();
  });
...
describe("PageSkeletonOrchestrator", () => {
  it("renders mixed loading and ready surfaces with page-level busy state", () => {
    render(
      <PageSkeletonOrchestrator
        surfaces={[
          {
            ...
            label: "Wallet",
            status: "loading",
            content: <div data-testid="wallet-ready">Wallet ready</div>,
            loadingFallback: (
              <div data-testid="wallet-loading">Wallet loading</div>
            ),
          },
          {
            id: "rewards",
            label: "Rewards",
            status: "ready",
            content: <div data-testid="rewards-ready">Rewards ready</div>,
          },
        ]}
      />,
    );

    expect(screen.getByTestId("page-skeleton-orchestrator")).toHaveAttribute(
      "aria-busy",
      "true",
    );
    expect(screen.getByTestId("page-skeleton-orchestrator")).toHaveAttribute(
    ...
    expect(screen.getByTestId("rewards-ready")).toBeInTheDocument();
    expect(screen.queryByTestId("wallet-ready")).not.toBeInTheDocument();
  });

  it("falls back to explicit error and empty surfaces", () => {
    render(
      <PageSkeletonOrchestrator
        surfaces={[
          {
            ...
            status: "error",
            content: <div />,
            errorFallback: <div data-testid="custom-error">Unavailable</div>,
          },
          {
            id: "empty",
            label: "Empty",
