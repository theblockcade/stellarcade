...
/**
 * Ported from frontend/src/i18n/provider.tsx. Behavior is unchanged; the
 * only edit is `import.meta.env.DEV` (Vite) -> `process.env.NODE_ENV`
 * (Next/Node) for the missing-translation dev warning.
 */
...
export function isSupportedLocale(value: unknown): value is Locale {
  return typeof value === "string" && Object.prototype.hasOwnProperty.call(LOCALE_TO_INTL, value);
}
...
export const I18nProvider: React.FC<I18nProviderProps> = ({
  ...
  const [messageRegistry, setMessageRegistry] = useState(STATIC_MESSAGES);
  ...
  React.useEffect(() => {
    const initializeMessages = async () => {
      ...
      const currentMessages = locale !== DEFAULT_LOCALE ? await loadMessages(locale) : defaultMessages;
