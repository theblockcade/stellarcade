...
/**
 * Ported from frontend/src/services/feature-flags.tsx. Only change:
 * `import.meta.env.DEV` / `import.meta.env.VITE_FEATURE_FLAG_OVERRIDES`
 * (Vite) -> `process.env.NODE_ENV` / `process.env.NEXT_PUBLIC_FEATURE_FLAG_OVERRIDES`
 * (Next requires the NEXT_PUBLIC_ prefix for a client-readable env var).
 */
...
export const FeatureFlagsProvider: React.FC<FeatureFlagsProviderProps> = ({ children, overrides = {} }) => {
  const envOverrides = useMemo(() => {
    if (process.env.NODE_ENV === "development") {
      return parseFlagOverrides(process.env.NEXT_PUBLIC_FEATURE_FLAG_OVERRIDES);
    }
    return {};
