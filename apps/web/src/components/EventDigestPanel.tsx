...
function DigestRow({ event, testId }: { event: DigestEvent; testId: string }): React.JSX.Element {
  const tone = SEVERITY_TONE[event.severity ?? "info"];
  return (
    <li className="edp__row" data-testid={`${testId}-row-${event.id}`}>
      ...
      {event.summary && <span className="edp__summary">{event.summary}</span>}
    </li>
  );
}
    ...
    >
      ...
      {!isLoading && !isError && visible.length > 0 && (
        <ul
          ...
          aria-label={`${visible.length} contract events`}
        >
          {visible.map((event) => (
...
EventDigestPanel.displayName = "EventDigestPanel";
